    ...
    <div className="rounded-2xl overflow-hidden border border-white/5 bg-[#16161a]">
      ...
      <div className="flex items-center gap-3 px-5 py-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {copied === `token-${token.id}` ? <><IconCheck /> Copied!</> : <><IconCopy /> Copy Link</>}
          </button>
          <button
            onClick={() => setOpen((v) => !v)}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button type="button" onClick={() => { const rand = Math.random().toString(36).substring(2, 9); setNewSlug(`auto-${rand}`); }}
